...
impl Component for Balance {
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        let total_balance =
            ctx.props().on_chain_confimed + ctx.props().on_chain_pending + ctx.props().ln;
        html! {
            <>
        <a class="block flex-1 p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
            ...
            <p class="font-normal text-gray-700 dark:text-gray-400">{format!("Onchain Spendable: {}", ctx.props().on_chain_confimed.to_sat())}</p>
